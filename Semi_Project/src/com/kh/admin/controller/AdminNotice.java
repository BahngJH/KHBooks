		...
		if (logined != null) {
			...
			int cPage;
				...
				if(pageNo ==1) {			
					...
					+"/admin/adminnotice?cPage="+(pageNo-1)
				...
				//선택페이지 만들기
						while(!(pageNo>pageEnd||pageNo>totalPage))
						{
							...
							{
								pageBar+="<li><a href='"+request.getContextPath()
								+"/admin/adminnotice?cPage="+(pageNo)
				...
				if(pageNo>totalPage) {
					...
					pageBar+="<a href='"+request.getContextPath()+"/admin/adminnotice?cPage="+pageNo+"&numPerPage="+numPerPage+"'>[다음]</a>";
				}
				...
				request.getRequestDispatcher("/views/admin/adminnotice.jsp").forward(request, response);
			}else {
				//관리자가 아닐 때
